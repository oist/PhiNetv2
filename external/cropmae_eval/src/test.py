...
def main(args):
    # finetune = "/work/YamadaU/myamada/Python/2025/RSP/checkpoint/rsp_vits16.pth"
    if args.model_type == 'vit_small16':
        args.map_scale = np.array([16, 16])
    elif args.model_type == 'vit_large16':
        args.map_scale = np.array([16, 16])

    # building network
    model = vits.__dict__[args.model_vit](patch_size=args.patch_size, num_classes=0)
    print(f"Model {args.model_vit} {args.patch_size}x{args.patch_size} built.")
    model.cuda()
        ...
        with torch.no_grad():
            ...
            for b in range(0, imgs.shape[1], bsize):
                if args.model_type == "scratch":
                    ...
                    feat_vit = model.get_intermediate_layers(imgs[0, b:b+bsize].to(args.device), n=1)[0]
