...
def clean_label(label):
    """ Clean the label name by removing unwanted characters """
    return (
        label.strip('"')  # Remove leading and trailing quotes
...
def load_label(csv):
    """ Load labels from CSV file and clean up class names """
    table = np.loadtxt(csv, skiprows=1, dtype=str, delimiter=',')
    return {k: clean_label(v) for k, v in zip(table[:, 1], table[:, 0])}
