...
class PhiNetv2(nn.Module):
    ...
    def forward_mae_loss(self, imgs, pred, mask=None):
    ...
    def forward(self, src_imgs, tgt_imgs, epoch):
        ...
        # MAE
        img_t, mask, ids_restore = self.forward_encoder(tgt_imgs, mask_ratio=self.mask_ratio)
        pred_masked_t = self.forward_decoder_mae(img_t, ids_restore)
        mae_loss_t = self.forward_mae_loss(tgt_imgs, pred_masked_t, mask)

        img_s, mask, ids_restore = self.forward_encoder(src_imgs, mask_ratio=self.mask_ratio)
        pred_masked_s = self.forward_decoder_mae(img_s, ids_restore)                        
        mae_loss_s = self.forward_mae_loss(src_imgs, pred_masked_s, mask)

        mae_loss = mae_loss_t/2 + mae_loss_s/2

        loss_sim2 = mseloss(tgt_pred,tgt_z[:,1:,:].detach())/2 + mseloss(src_pred,src_z[:,1:,:].detach())/2

        loss = loss_sim2 + self.kl_scale*loss_sim1 + mae_loss 
...
def phinetv2_vit_large_patch14_dec512d8b(**kwargs):
    model = PhiNetv2(
        patch_size=14,
        embed_dim=768,
        depth=24,
...
phinetv2_vit_large_patch14 = phinetv2_vit_large_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
